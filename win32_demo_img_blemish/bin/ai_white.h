...
{
	AI_EXPORT_WHITE void f_write_line( BYTE *bmp,BYTE *bmp_fy,int img_w, int img_h);//提取辅助线坐标，写入d：\\coord.txt
	AI_EXPORT_WHITE void f_read_line(BYTE *bmp,BYTE *bmp_f,int img_w, int img_h);//读取coord.txt，滤除辅助线
//	AI_EXPORT_WHITE void filter_line( BYTE *bmp,BYTE *bmp_fy,int img_w, int img_h, char* v);
}

...
{
	ai_win::read_string_key(s_aa::config_path, "system", "default_recipe_setting", s_aa::recipe_name, sizeof(s_aa::recipe_name));
	SetDlgItemText(hdlg, IDC_SELECTED_RECIPE, s_aa::recipe_name);

	char path[MAX_PATH] = {0};
	sprintf(path, "%s\\recipe\\", s_aa::dir);
	...
	for (int i=0; i<folder_cnt; i++){
		...
		if (!strcmp(folder_name[i], s_aa::recipe_name)){
			index = i;
		}
	}
	...
	sprintf(title_name, "Recipe Select: [%s]", s_aa::recipe_name);
	SetWindowText(hdlg, title_name);

	char file_name[50][FOLDER_NAME_LENGTH] = {0};
	sprintf(path, "%s\\recipe\\%s\\", s_aa::dir, s_aa::recipe_name);
	...
	log("Dlg:recipe setting init OK, select recipe[%s]", s_aa::recipe_name);
	return TRUE;
}
	...
	case IDC_CANCEL:
		{
			...
			char path[MAX_PATH] = {0};
			sprintf(path, "%s\\recipe\\%s\\%s", s_aa::dir, s_aa::recipe_name, name);
...
void RecipeDlg::close(HWND hdlg)
{
	...
	CheckMenuItem(s_aa::hmenu, ID_SETTING_RECIPE, MF_UNCHECKED);
	EndDialog(hdlg, FALSE);
}

void RecipeDlg::change_recipe(HWND hdlg, char *recipe_name)
{
	if (strcmp(recipe_name, s_aa::recipe_name) != 0x00){
		strcpy(s_aa::recipe_name, recipe_name);
		ai_win::write_string_key(s_aa::config_path, "system", "default_recipe_setting", s_aa::recipe_name);
		...
		log("Change recipe to [%s] success.", s_aa::recipe_name);
	}
	else{

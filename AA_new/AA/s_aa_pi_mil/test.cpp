	...
	unsigned width, unsigned height, unsigned bytesWidth, void* context)
{
	...
	s_aa::cimg.yuv422_to_bmp24(0, buffer, s_aa::bmp_buf, width, height);
...
Test::Test()
{
	...
	for (int i=0; i<TEST_ITEM_NUM; i++){
		s_aa::test_option[i].on = FALSE;
		s_aa::test_option[i].processed = FALSE;
		s_aa::test_option[i].draw_result_on = FALSE;
		s_aa::test_option[i].order = 0;
	}
}
...
int Test::get_next_item()
{
	...
	for (int i=0; i<TEST_ITEM_NUM; i++){
		if (s_aa::test_option[i].on && s_aa::test_option[i].processed == FALSE){
			if (min_index>s_aa::test_option[i].order){
				min_index = s_aa::test_option[i].order;
				next_item_index = i;
			}
		}
	}
...
void Test::set_item_tested(int item_id)
{
	if (item_id>=0 && item_id<TEST_ITEM_NUM){
		s_aa::test_option[item_id].processed = TRUE;
	}
}
...
void Test::draw_test_item(int item_id)
{
	for (int i=0; i<TEST_ITEM_NUM; i++){
		if (i == item_id){
			s_aa::test_option[i].draw_result_on = TRUE;
		}
		else s_aa::test_option[i].draw_result_on = FALSE;
	}
}
...
DWORD CALLBACK test_thread(LPVOID temp)
{
	...
	int result = FALSE;
//	RECT rt;
	if (s_aa::test.create_event() != TRUE){
		ERROR_ON = TRUE;
		ERROR_CODE = ERR_CREATE_TEST_EVENT;
	}

	if (s_aa::test.create_mutex() != TRUE){
		ERROR_ON = TRUE;
	...
	int test_item_id = 0;
	while (1){		
		if(SELECT_IMG_SOURCE == IMG_DEV_MIL&&!do_close)
			s_aa:: cap_dev->stop();
		...
		if (IMG_DEVICE_INITED == FALSE){
			...
			result = s_aa::cap_dev->init(TESTER_ID);
			log("init done.result = %d",result);
			if (result == TRUE){
				result = s_aa::cap_dev->run();
			}

			log("run done.result = %d",result);
			if (result == TRUE){
				IMG_DEVICE_INITED = TRUE;
				result = s_aa::test.set_test_on();
				log("set test on.");
			}
			else{
				ERROR_ON = TRUE;
	...
	//	log("test start active.");
		DWORD event_rt = WaitForSingleObject(s_aa::test.get_event(), INFINITE);
		switch (event_rt){
		case WAIT_OBJECT_0:
			if (TEST_START == FALSE){
				continue;
		...
		if (s_aa::test.get_test_on() == TRUE){
			s_aa::test.set_test_stop();
			timeout = 100;
			while (s_aa::test.get_test_on() == TRUE && timeout-->0){
				Sleep(100);
			}
		...
		test_item_id = s_aa::test.get_next_item();
		...
		ResetEvent(s_aa::test.get_event());
		Sleep(10);
	}
...
DWORD CALLBACK get_image_thread(LPVOID temp)
{
	...
	while (1){
		...
		while (s_aa::test.get_test_stop() == FALSE){
			if(SELECT_IMG_SOURCE != IMG_DEV_MIL)
				s_aa::cap_dev->cap(s_aa::bmp_buf_t,0);
			else
				s_aa::cap_dev->cap(s_aa::img_buf,0);
		}
		//log("no cap.");
		START_DISPLAY_IMAGE = FALSE;
		s_aa::test.close_mutex();	
		s_aa::test.set_test_off();
	}
...
DWORD CALLBACK display_image_thread(LPVOID temp)
{
	...
	while (1){
		...
		wait_rt = WaitForSingleObject(s_aa::test.get_mutex(), 1000);
		...
		s_aa::test.set_test_on();
		s_aa::test.set_item_tested(TI_DISPLAY_IMAGE);
		//////////////////////////////////////////////////////////////////////////
		//get image
		log("Img w:%d, h:%d", DISPLAY_IMG_WIDTH, DISPLAY_IMG_HEIGHT);
		while (s_aa::test.get_test_stop() == FALSE){
			...
			if(SELECT_IMG_SOURCE == IMG_SOURCE_EXTERN_DEVICE){
				YCbCrITUToBmp(para.output_sequence, s_aa::img_buf, s_aa::bmp_buf_t, IMG_WIDTH, IMG_HEIGHT);
			}
			else
				YCbCrITUToBmp(1, s_aa::img_buf, s_aa::bmp_buf_t, IMG_WIDTH, IMG_HEIGHT);//tsw306e
			}
		//	log("mirror_x=%d,mirror_y=%d",IMG_MIRROR_X,IMG_MIRROR_Y);
			if(IMG_MIRROR_X){
				if(IMG_MIRROR_Y){
					for (int y=0; y<IMG_HEIGHT; y++){
						for (int x=0; x<IMG_WIDTH; x++){
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+x*3];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+1] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+x*3+1];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+2] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+x*3+2];
						}
					}
				}
				else{
					for (int y=0; y<IMG_HEIGHT; y++){
						for (int x=0; x<IMG_WIDTH; x++){
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+x*3];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+1] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+x*3+1];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+2] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+x*3+2];
						}
					}
				}
			}
				...
				if(IMG_MIRROR_Y){
					for (int y=0; y<IMG_HEIGHT; y++){
						for (int x=0; x<IMG_WIDTH; x++){
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+1] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3+1];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+2] = s_aa::bmp_buf_t[y*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3+2];
						}
					}
				}
				else{
					for (int y=0; y<IMG_HEIGHT; y++){
						for (int x=0; x<IMG_WIDTH; x++){
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+1] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3+1];
							s_aa::bmp_buf[y*3*IMG_WIDTH+x*3+2] = s_aa::bmp_buf_t[(IMG_HEIGHT-1-y)*3*IMG_WIDTH+(IMG_WIDTH-1-x)*3+2];
						}
					}
				}
			}
			s_aa::myimg.clear_bmp_stream();
			s_aa::myimg.display(GetDlgItem(HWND_MAIN, IDC_DISPLAY), 
							   s_aa::bmp_buf, 
							   DISPLAY_IMG_WIDTH, 
							   DISPLAY_IMG_HEIGHT, 
							   IMG_DEPTH/*,
...
DWORD CALLBACK reg_mark_chart_test_thread(LPVOID temp)
{
	...
	//	s_aa::test.set_item_tested(TI_REG_MARK_CHART);
		//////////////////////////////////////////////////////////////////////////
		//init test
		result = init_reg_mark_chart_test();
		log("init reg result: %d",result);

		if (s_aa::test_option[TI_REG_MARK_CHART].mode == 0x00){ //loop display
			log("rm w=%d, h=%d, img source=%d", IMG_WIDTH, IMG_HEIGHT,SELECT_IMG_SOURCE);
			while (s_aa::test.get_test_stop() == FALSE){
				if(SELECT_IMG_SOURCE == IMG_DEV_FILE){
 			 		memcpy(s_aa::img_buf, s_aa::bmp_buf_t, IMG_WIDTH*IMG_HEIGHT*2);
				}
			//	log("trans bmp.");
			...
			//	log("trans2 bmp.");
				exec_reg_mark_chart_test(NULL, s_aa::bmp_buf, IMG_WIDTH, IMG_HEIGHT);
				SfrResult(3);//画面显示红绿判断
				s_aa::test.draw_test_item(TI_REG_MARK_CHART);
			//	log("trans3 bmp.");
				IMG_CAPTURE_READY = TRUE;
				Sleep(1);
			}

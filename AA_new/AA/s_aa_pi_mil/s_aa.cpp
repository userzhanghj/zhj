...
INIEdit config_dlg[] = {
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.rt_win.left,			"win_left"		},
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.rt_win.top,			"win_top"		},
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.rt_win.right,			"win_right"		},
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.rt_win.bottom,			"win_bottom"	},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.mdlg_x,				"mdlg_x"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.mdlg_y,				"mdlg_y"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.rt_log.left,			"log_left"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.rt_log.top,			"log_top"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.rt_log.right,			"log_right"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.rt_log.bottom,			"log_bottom"	},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.aadlg_x,				"aadlg_x"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.aadlg_y,				"aadlg_y"		},

	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.tcdlg_x,				"tcdlg_x"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.tcdlg_y,				"tcdlg_y"		},
	{NULL,	DLGTYPE_INPUT_INT,	&s_aa::dinfo.show_log,				"show_log"		},
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.show_tc,				"show_tc"		},
	{NULL,  DLGTYPE_INPUT_INT,  &s_aa::dinfo.show_aa_dlg,			"show_aa_dlg"	},
...
{
	s_aa::dinfo.rt_log.left = 1000;
	s_aa::dinfo.rt_log.top = 250;
	s_aa::dinfo.rt_log.right = s_aa::dinfo.rt_log.left+305;
	s_aa::dinfo.rt_log.bottom = s_aa::dinfo.rt_log.top+545;

	s_aa::dinfo.rt_win.left = 100;
	s_aa::dinfo.rt_win.top = 100;
	s_aa::dinfo.rt_win.right = 1000;
	s_aa::dinfo.rt_win.bottom = 800;

	s_aa::dinfo.mdlg_x = 106;
	s_aa::dinfo.mdlg_y = 104;

	s_aa::dinfo.tcdlg_x = 200;
	s_aa::dinfo.tcdlg_y = 100;

	s_aa::dinfo.aadlg_x = 100;
	s_aa::dinfo.aadlg_y = 100;
	
	s_aa::dinfo.show_img_info  = FALSE;
	s_aa::dinfo.show_recipe	  = FALSE;
	s_aa::dinfo.show_tc		  = FALSE;
	s_aa::dinfo.show_hw_config = FALSE;
	s_aa::dinfo.show_aa_dlg    = FALSE;
}

int WINAPI WinMain(__in HINSTANCE hInstance, __in_opt HINSTANCE hPrevInstance, __in LPSTR lpCmdLine, __in int nShowCmd)
{
	//HOME_STATUS=0;
	memset(s_aa::dir, 0, sizeof(s_aa::dir));
	ai_win::get_current_directory(s_aa::dir);
	...
	ai_win::load_keys(s_aa::config_path, "dlg", NULL, config_dlg, sizeof(config_dlg)/sizeof(config_dlg[0]));

	if (s_aa::dinfo.rt_log.left>cx || s_aa::dinfo.rt_log.top>cy || s_aa::dinfo.rt_log.left<0 || s_aa::dinfo.rt_log.top<0 ||
		s_aa::dinfo.rt_win.left>cx || s_aa::dinfo.rt_win.top>cy || s_aa::dinfo.rt_win.left<0 || s_aa::dinfo.rt_win.top<0 ||
		s_aa::dinfo.mdlg_x>cx || s_aa::dinfo.mdlg_y>cy || s_aa::dinfo.mdlg_x<0 || s_aa::dinfo.mdlg_y<0 ||
		s_aa::dinfo.tcdlg_x>cx || s_aa::dinfo.tcdlg_y>cy || s_aa::dinfo.tcdlg_x<0 || s_aa::dinfo.tcdlg_y<0){
		dlg_size_reset();
	}
	...
	HWND_APP = CreateWindowEx(WS_EX_ACCEPTFILES, 
		...
		WS_THICKFRAME ,
		s_aa::dinfo.rt_win.left,
		s_aa::dinfo.rt_win.top,
		(s_aa::dinfo.rt_win.right-s_aa::dinfo.rt_win.left),
		(s_aa::dinfo.rt_win.bottom-s_aa::dinfo.rt_win.top),
		NULL, 
		NULL, 
		hInstance, 
		NULL);
...
//  	UpdateWindow(HWND_APP);
	...
	MoveWindow(HWND_MAIN, s_aa::dinfo.mdlg_x, s_aa::dinfo.mdlg_y, rt_mdlg.right-rt_mdlg.left, rt_mdlg.bottom-rt_mdlg.top, FALSE);
...
void Wnd::hscroll(HWND hwnd, HWND hctrl, UINT code, int pos)
{
	...
	s_aa::myimg.set_start_x(si.nPos);
	...
	if (si.nPos != s_aa::myimg.get_start_x()){
...
void Wnd::vscroll(HWND hwnd, HWND hctrl, UINT code, int pos)
{
	...
	s_aa::myimg.set_start_y(si.nPos);
	...
	if (si.nPos != s_aa::myimg.get_start_y()){
		InvalidateRect(hwnd, NULL, FALSE);
		UpdateWindow(hwnd);
	}
}

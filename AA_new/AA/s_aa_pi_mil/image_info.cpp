...
BOOL ImageInfoDlg::init(HWND hdlg, HWND hfocus, LPARAM linit)
{
	...
	sprintf(config_path, "%s\\recipe\\config.ini", s_aa::dir);
	...
	//////////////////////////////////////////////////////////////////////////
	//set image para
	s_aa::myimg.set_start_x(0);
	s_aa::myimg.set_start_y(0);
	s_aa::myimg.set_full_sized(FALSE);
...
{
	CheckMenuItem(s_aa::hmenu, ID_EDIT_IMAGEINFO, MF_UNCHECKED);
	EndDialog(hdlg, TRUE);
}
...
DWORD CALLBACK image_info_thread(LPVOID temp)
{
	...
	while (1){
		...
		SetDlgItemText(HWND_IMG_INFO, IDC_IMAGE_FORMAT, s_aa::img_format_str[IMG_FORMAT]);

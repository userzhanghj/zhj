...
void Histogram::show_histogram(HWND hdlg)
{
	...
	if (GET_CHECKER(hdlg, IDC_SELECT_RGB_24BIT)){
		...
		memcpy(af_bmp,s_aa::bmp_buf, cx*cy*3);
		s_aa::cimg.bmp24_to_y8(af_bmp, cx, cy, NULL, y_buf);
		...
		if (IMG_RAW_FORMAT == RAW_8BIT){
			memcpy(raw8, s_aa::img_buf, IMG_WIDTH*IMG_HEIGHT);
		}
		else if (IMG_RAW_FORMAT == RAW_10BIT){
			ai_img::raw10_to_raw8(s_aa::img_buf, raw8, IMG_WIDTH, IMG_HEIGHT);
		}
		else if (IMG_RAW_FORMAT == RAW_12BIT){
			ai_img::raw12_to_raw8(s_aa::img_buf, raw8, IMG_WIDTH, IMG_HEIGHT);
		}
		else{

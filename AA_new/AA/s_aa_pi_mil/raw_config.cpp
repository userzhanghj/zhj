...
BOOL RawConfigDlg::init(HWND hdlg, HWND hfocus, LPARAM linit)
{
	...
	ai_win::read_string_key(s_aa::config_path, "img", "width_pair", buf, sizeof(buf));
	int select_w = ai_win::read_integer_key(s_aa::config_path, "img", "select_width");
	...
	ai_win::read_string_key(s_aa::config_path, "img", "height_pair", buf, sizeof(buf));
	int select_h = ai_win::read_integer_key(s_aa::config_path, "img", "select_height");
	token = strtok(buf, sep);
	...
	int enable_customized_size = ai_win::read_integer_key(s_aa::config_path, "img", "enable_customized_img_size");
	SET_CHECKER(hdlg, IDC_ENABLE_CUSTOMIZED_SIZE, enable_customized_size);
	int customized_img_w = ai_win::read_integer_key(s_aa::config_path, "img", "customzied_img_w");
	int customzied_img_h = ai_win::read_integer_key(s_aa::config_path, "img", "customzied_img_h");
	...
	index = 0;
	memset(buf, 0, sizeof(buf));
	ai_win::read_string_key(s_aa::config_path, "img", "header_pair", buf, sizeof(buf));
	int select_header = ai_win::read_integer_key(s_aa::config_path, "img", "select_header");
	token = strtok(buf, sep);
	...
	IMG_PIXEL_ORDER = ai_win::read_integer_key(s_aa::config_path, "img", "pixel_oder");
	...
	IMG_RAW_FORMAT = ai_win::read_integer_key(s_aa::config_path, "img", "raw_format");
	...
	IMG_SELECT_DEMOSAIC = ai_win::read_integer_key(s_aa::config_path, "img", "select_demosaic");
			...
			if (result == TRUE){
				if (result == TRUE){
					...
					if (result == TRUE){
						...
						s_aa::myimg.display(GetDlgItem(hdlg, IDC_DISPLAY), mybmp, IMG_WIDTH, IMG_HEIGHT, IMG_DEPTH);
...
int RawConfigDlg::set_raw_config(HWND hdlg)
{
	...
	ai_win::write_integer_key(s_aa::config_path, "img", "pixel_oder",	  IMG_PIXEL_ORDER);
	ai_win::write_integer_key(s_aa::config_path, "img", "raw_format",	  IMG_RAW_FORMAT);
	ai_win::write_integer_key(s_aa::config_path, "img", "select_demosaic", IMG_SELECT_DEMOSAIC);
	...
	if (GET_CHECKER(hdlg, IDC_ENABLE_CUSTOMIZED_SIZE)){
		...
		ai_win::write_integer_key(s_aa::config_path, "img", "customzied_img_w", IMG_WIDTH);
		ai_win::write_integer_key(s_aa::config_path, "img", "enable_customized_img_size", TRUE);
	}
	else{
		IMG_WIDTH = atoi(buf);
		ai_win::write_integer_key(s_aa::config_path, "img", "select_width",	  index);
		ai_win::write_integer_key(s_aa::config_path, "img", "enable_customized_img_size", FALSE);
	}
	...
	if (GET_CHECKER(hdlg, IDC_ENABLE_CUSTOMIZED_SIZE)){
		...
		ai_win::write_integer_key(s_aa::config_path, "img", "customzied_img_h", IMG_HEIGHT);
	}
	else{
		IMG_HEIGHT = atoi(buf);
		ai_win::write_integer_key(s_aa::config_path, "img", "select_height",	  index);
	}
	memset(buf, 0, sizeof(buf));
	index = COMBO_GETCURSEL(hdlg, IDC_RAW_HEAD_OFFSET);
	COMBO_GETTEXT(hdlg, IDC_RAW_HEAD_OFFSET, index, buf);
	IMG_RAW_HEADER = atoi(buf);
	ai_win::write_integer_key(s_aa::config_path, "img", "select_header",	  index);

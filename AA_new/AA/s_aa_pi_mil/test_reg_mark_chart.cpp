...
// 	{IDC_SFR_Y_END,			DLGTYPE_INPUT_INT,		&mysbw.sfr_chartsize[3],	"sfr_y_end"},    调整sfr和fs曲线绘图
...
// 		}
	...
	DeleteObject(hfont);
// 	hfont	 = CreateFont(50, 0,0,0,20,0,0,0,ANSI_CHARSET,3,2,1, VARIABLE_PITCH | FF_ROMAN, "黑体");
...
void save_reg_mark_report_test(int n,int ty)//save  MTF
{
	...
	if(ty==1){
		...
		is_report_exist = create_report_file_test("SCAN_拟合", report_folder_path, report_folder_name);
		sprintf(report_path, "%s\\SCAN_拟合_report_%s.txt", report_folder_path, mysbw.sn);
	}
	else{
		is_report_exist = create_report_file_test("MTF曲线", report_folder_path, report_folder_name);
		sprintf(report_path, "%s\\MTF曲线_report_%s.txt", report_folder_path, mysbw.sn);
	}
	HANDLE report_handle;
	...
	if (is_report_exist == FALSE){
		SetFilePointer(report_handle, 0, NULL, FILE_BEGIN);
		if(ty==1)
			sprintf(str, "## MTF SCAN 曲线 ## \r\ncy/px = 0.25\r\n偏心X坐标	偏心Y坐标	Z轴坐标	中心	左上	右上	左下	右下\r\n");
		else if(ty==3)
			sprintf(str, "## AA 完成后 UV 前 ## \r\ncy/px = 0.25\r\n偏心X坐标	偏心Y坐标	Z轴坐标	中心	左上	右上	左下	右下\r\n");
		else if(ty==4)
			sprintf(str, "## AA 完成后 UV 后 ## \r\ncy/px = 0.25\r\n偏心X坐标	偏心Y坐标	Z轴坐标	中心	左上	右上	左下	右下\r\n");
		else if(ty==5)
			sprintf(str, "## MTF SCAN 拟合曲线 ## \r\n	中心	左上	右上	左下	右下\r\n");
		else
			sprintf(str, "## MTF 曲线 ## \r\ncy/px	SFR\r\n");
		ai_win::write_csv(report_handle, str);
	}
...
void save_reg_mark_report_test2(int ty/*MES or LOCAL*/,int time_num/*time for save*/)//save MES and LOCAL MTF
{
	...
	//else if(ty==5){
	//	is_report_exist = create_report_file_test("SCAN_拟合", report_folder_path, report_folder_name);
	//	sprintf(report_path, "%s\\SCAN_拟合_report_%s_%02d.txt", report_folder_path, report_folder_name,aa_num);
	}
	else{
	...
	if (ty == 1){
		SetFilePointer(report_handle, 0, NULL, FILE_END);
		if(time_num==1)
			sprintf(str, "## AA完成后 补偿前 ##\r\n");
		else if(time_num==2)
			sprintf(str, "## AA完成后 补偿后 ##\r\n");
		else if(time_num==3)
			sprintf(str, "## UV完成后 松开夹爪前 ##\r\n");
		else if(time_num==4)
			sprintf(str, "## UV完成后 松开夹爪后 ##\r\n");
		ai_win::write_csv(report_handle, str);
	}
	else{
		SetFilePointer(report_handle, 0, NULL, FILE_END);
	...
	SetFilePointer(report_handle, 0, NULL, FILE_END);
	if(ty==1){
		sprintf(str,"偏心X坐标：%.2f	偏心Y坐标：%.2f		Z轴坐标：%.4f\r\n",mysbw.oc_x[0],mysbw.oc_y[0],Pos2[2]);
		ai_win::write_csv(report_handle, str);
		for(int i = 0;i < 5; i++){
			val = 0;
			n = 0;
		...
		//测试项名称，测试值，下限，上限，描述， 状态（P:成功、F:失败）
